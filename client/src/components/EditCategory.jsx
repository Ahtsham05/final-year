...
const EditCategory = ({close, data : editData,fetch}) => {
        ...
        <div className='bg-white w-full max-w-md p-2'>
            ...
            <form onSubmit={handleSubmit} className='grid gap-4 mt-5'>
                    <div className='grid gap-1'>
                        ...
                        <input type="text" name='name' value={data.name} onChange={handleChange} placeholder='Enter Category Name'  className='border bg-blue-50 outline-none p-2 focus-within:border-primary200 rounded'/>
                    </div>
                    ...
                    <div className='grid'>
                        <button disabled={!ValidateFields} className={`py-2 border border-primary200 rounded hover:bg-primary200 hover:text-white ${!ValidateFields && "bg-gray-400 border-none hover:bg-gray-400 hover:text-neutral-700"}`}>Update Category</button>
                    </div>
            </form>
        </div>
    </section>
